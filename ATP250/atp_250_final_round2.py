...
def get_player_name_fourth_round_winner32(soup):
...
def get_final_round_winner32(soup):
    colonne6 = soup.find("td", "col_6")

    player = colonne6.find("div", "playerWrap")
    score = colonne6.find("div", "scores")

    return player.a.string, score.a.string


def get_player_name_fifth_round_winner64(soup):
    colonne6 = soup.find("td", "col_6")
...
def get_final_round_winner64(soup):
    colonne7 = soup.find("td", "col_7")

    player = colonne7.find("div", "playerWrap")
    score = colonne7.find("div", "scores")
    ...
    for tournamentYear_url in tournamentYear_urls_clean:
        ...
        if get_tournament_date(soup) != 2:
            ...
            if get_tournament_draw(soup) == 32:
                fourth_round_winner = get_player_name_fourth_round_winner32(soup)
                size = get_final_round_winner32(soup)
                winner, score = get_final_round_winner32(soup)
            elif get_tournament_draw(soup) == 64:
                fourth_round_winner = get_player_name_fifth_round_winner64(soup)
                size = get_final_round_winner64(soup)
                winner, score = get_final_round_winner64(soup)
            ...
            while i < len(size):
                ...
                #winner, score = get_final_round_winner(soup)
