...
def get_player_name_second_round_winner32(soup):
...
def get_player_name_third_round_winner64(soup):
...
#     player = colonne5.find("div", "playerWrap")
#     score = colonne5.find("div", "scores")

#     return player.a.string, score.a.string

def get_third_round_winner32(soup):
    colonne4 = soup.find("td", "col_4")
...
def get_fourth_round_winner64(soup):
    colonne5 = soup.find("td", "col_5")
...
while l < len(tournamentList):
    ...
    while r32 < len(tournamentYear_urls32):
        ...
        for tournamentYear_url in tournamentYear_urls32:
            ...
            second_round_winner = get_player_name_second_round_winner32(soup)
            size = get_third_round_winner32(soup)[0]
            winner, score = get_third_round_winner32(soup)
            draw = get_tournament_draw(soup)
            round_name = "Third Round"
            file_name = "_Third_Round"
    ...
    while r64 < len(tournamentYear_urls64):
        ...
        for tournamentYear_url in tournamentYear_urls64:
            ...
            third_round_winner = get_player_name_third_round_winner64(soup)
            size = get_fourth_round_winner64(soup)[0]
            winner, score = get_fourth_round_winner64(soup)
            draw = get_tournament_draw(soup)
            round_name = "Fourth Round"
            file_name = "_Fourth_Round64"
            ...
            if get_tournament_date(soup) != 2:
                ...
                while i < len(size):
                    ...
                    rank1 = get_player_specific_rank(soup2, player1)
                    rank2 = get_player_specific_rank(soup2, player2)
                    mon_fichier.write('{Tournament};{Location};{Surface};{tournamentYear};{Prize};{Round};{Rank1};{Player1};{Nation1};{Rank2};{Player2};{Nation2};{Gagnant};{Score}'.format(
                        Tournament=name_tournament,
                        Location=tournament_location,
                        Surface=surface,
                        tournamentYear=get_tournament_date(soup)[0],
