...
#tournamentList = ['404', '403', '410', '1536', '416', '421', '422', '5014', '352']
...
def make_player_ranking(soup, soup3):
    html = str(soup.findAll("td", "first"))
    html3 = str(soup3.findAll("td", "first"))
    soup = BeautifulSoup(html, "html.parser")
    soup3 = BeautifulSoup(html3, "html.parser")
...
def get_player_specific_rank(dico, player):
    try:
        rank = dico[player]
        return rank
    except KeyError:
...
def make_liste_nations(soup, soup3):
    html = str(soup.findAll("td", "first"))
    html3 = str(soup3.findAll("td", "first"))
    soup = BeautifulSoup(html, "html.parser")
    soup3 = BeautifulSoup(html3, "html.parser")
...
def get_player_specific_nation(dico, player):
    try:
        nation = dico[player]
        return nation
    except KeyError:
...
def get_player_name_first_round64(soup):
    colonne1 = soup.find("td", "col_2")
...
def get_player_name_first_round_winner128(soup):
    colonne2 = soup.find("td", "col_3")
...
def get_first_round_winner64(soup):
    colonne2 = soup.find("td", "col_3")
...
def get_second_round_winner128(soup):
    colonne3 = soup.find("td", "col_4")
...
while l < len(tournamentList):
    ...
    for tournamentYear_url in tournamentYear_urls_clean:
        soup = make_soup(tournamentYear_url)
        if get_tournament_draw(soup) <= 64:
            tournamentYear_urls32.append(tournamentYear_url)
        elif get_tournament_draw(soup) == 128:
    ...
    while r32 < len(tournamentYear_urls32):
        mon_fichier = open("Masters1000_1996_2013_4", "a")
        ...
        for tournamentYear_url in tournamentYear_urls32:
            ...
            first_round_winner = get_player_name_first_round64(soup)
            size = get_first_round_winner64(soup)[0]
            winner, score = get_first_round_winner64(soup)
            draw = get_tournament_draw(soup)
            round_name = "Second Round"
            file_name = "_Second_Round"
            ...
            if get_tournament_date(soup) != 2 or get_tournament_draw(soup) != 2:
                ...
                URL2 = "http://www.atpworldtour.com/Rankings/Singles.aspx?d=" + str(get_closest_tournament_date(soup))
                
                # # On construit une url menant au classement atp qui coïncide le plus avec le tournoi (101 - 200)
                URL3 = "http://www.atpworldtour.com/Rankings/Singles.aspx?d=" + str(get_closest_tournament_date(soup)) + "&r=101"
                ...
                dico_nations = make_liste_nations(soup2, soup3)
                dico_ranking = make_player_ranking(soup2, soup3)
                ...
                while i < len(size):
                    ...
                    try:
                        ...
                        player2 = "None"
                    nation1 = get_player_specific_nation(dico_nations, player1)
                    nation2 = get_player_specific_nation(dico_nations, player2)
                    #winner, score = get_first_round_winner32(soup)
                    prize = get_tournament_prizemoney(soup)
                    rank1 = get_player_specific_rank(dico_ranking, player1)
                    rank2 = get_player_specific_rank(dico_ranking, player2)
    ...
    while r64 < len(tournamentYear_urls64):
        ...
        for tournamentYear_url in tournamentYear_urls64:
            ...
            second_round_winner = get_player_name_first_round_winner128(soup)
            size = get_second_round_winner128(soup)[0]
            winner, score = get_second_round_winner128(soup)
            draw = get_tournament_draw(soup)
            round_name = "Third Round"
            file_name = "_Third_Round128"
